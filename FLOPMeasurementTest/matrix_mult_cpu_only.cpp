...
int main()
{
    ...
    long cache_line_size = sysconf(_SC_LEVEL3_CACHE_LINESIZE);
    // If no sysconf -> assume 64? or C++ destructive interference size
    int retval = PAPI_create_eventset(&CPUEventSet);
    checkPAPIError(retval);
    retval = PAPI_add_event(CPUEventSet, PAPI_FP_OPS);
    checkPAPIError(retval);
    //retval =  PAPI_add_event(CPUEventSet, PAPI_L3_TCM);
    //checkPAPIError(retval);
    retval =  PAPI_add_event(CPUEventSet, PAPI_L3_TCM);
    checkPAPIError(retval);
    ...
    std::cout << "CPU GFLOP/s (From PAPI Instructions): " << static_cast<double>(CPUCounters[0]) * 1e-3 / static_cast<double>(duration.count()) << std::endl;
    std::cout << "CPU GBytes Accessed (Hand-counted): " << (static_cast<double>(bytes_accessed_per_mult) * numElements * 1e-9) << std::endl;
    std::cout << "CPU GBytes Accessed (Approximate L3): " << static_cast<double>(CPUCounters[1] * cache_line_size) * 1e-3 / static_cast<double>(duration.count()) << std::endl;
